...
describe('postcss-vars', function () {
    ...
    // LESS
    it('[LESS] replaces variables in values', function () {
        test('@size: 10px;\na{ width: @size; height: @size }',
            'a{ width: 10px; height: 10px }', {prefix : '@'});
    });

    it('[LESS] allows dashes and digits in variable name', function () {
        test('@a-b_10: 1;\na{ one: @a-b_10 a@(a-b_10) }', 'a{ one: 1 a1 }', {prefix : '@'});
    });

    it('[LESS] needs space before variable', function () {
        test('@size: 10px; a { width: one@size }', 'a { width: one@size }', {prefix : '@'});
    });

    it('[LESS] does not remove first symbol', function () {
        test('a{ a: 1 @a }', 'a{ a: 1 1 }', { variables: { a: 1 }, prefix : '@' });
    });

    it('[LESS] allows to use in negative numbers', function () {
        test('a{ a: -@a }', 'a{ a: -1 }', { variables: { a: 1 }, prefix : '@' });
    });

    it('[LESS] replaces multiple variables', function () {
        test('a{ a: @a @a }', 'a{ a: 1 1 }', { variables: { a: 1 }, prefix : '@' });
    });

    it('[LESS] has second syntax for varibles', function () {
        test('@size: 10; a { width: @(size)px }', 'a { width: 10px }', {prefix : '@'});
    });

    it('[LESS] overrides variables', function () {
        test('@var: 1; a{ one: @var } b{ @var: 2; two: @var } c{ two: @var }',
            'a{ one: 1 } b{ two: 2 } c{ two: 2 }', {prefix : '@'});
    });

    it('[LESS] throws an error on unknown variable', function () {
        expect(function () {
            test('a{ width: -@size }', {prefix : '@'});
        }).to.throw('<css input>:1:4: Undefined variable @size');
    });

    it('[LESS] allows to silent errors', function () {
        test('a{ width: @size }', 'a{ width: @size }', { silent: true, prefix : '@' });
    });

    it('[LESS] gets variables from options', function () {
        test('a{ width: @one }', 'a{ width: 1 }', { variables: { one: 1}, prefix : '@' });
    });

    it('[LESS] cans get variables only from option', function () {
        test('@one: 2; @two: 2; a{ one: @one @two }',
            '@one: 2; @two: 2; a{ one: 1 @two }',
            { only: { one: 1 }, prefix : '@' });
    });

    it('[LESS] works with false value', function () {
        test('a{ zero: @zero }', 'a{ zero: 0 }', { variables: { zero: 0 }, prefix : '@' });
    });

    it('[LESS] allows to use var in other vars', function () {
        test('@one: 1; @two: @one 2; a{ value: @two }', 'a{ value: 1 2 }', {prefix : '@'});
    });

    it('[LESS] set default values by function', function () {
        var value;
        var config = function () {
            return { config: value };
        };
        ...
        value = 1;
        test('a{ width: @config }', 'a{ width: 1 }', { variables: config, prefix : '@' });

        value = 2;
        test('a{ width: @config }', 'a{ width: 2 }', { variables: config, prefix : '@' });
    });
